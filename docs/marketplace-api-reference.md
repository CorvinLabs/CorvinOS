...
`BASE` is a **root-relative path**, not an absolute URL. `new URL(\`${BASE}/...\`)`
therefore throws `TypeError: Failed to construct 'URL': Invalid URL` — which a
`try/catch` around a fetch turns into an empty panel with a cryptic error string
instead of a network failure. Build query strings with `URLSearchParams` and
concatenate, or pass `window.location.origin` as the `URL` base.

## Marketplace Index Source

`GET /plugins` and `GET /plugins/{id}` are served from `plugins.json`, the index
generated by `generate_index_v2.py`. ADR-0512 moved the plugin sources and the
index out of CorvinOS into the sibling **Corvin-Marketplace** repository, so the
console resolves the index in this order
(`corvin_console.routes.marketplace.resolve_index_path`):

| # | Source | Notes |
|---|---|---|
| 1 | `$CORVIN_MARKETPLACE_INDEX` | Explicit operator override — an absolute path to a `plugins.json`. |
| 2 | `<cwd>/operator/marketplace/index/plugins.json` | Legacy in-repo index; only exists on branches predating ADR-0512. |
| 3 | `<repo parent>/Corvin-Marketplace/index/plugins.json` | Sibling checkout, the normal developer layout. |
| 4 | `raw.githubusercontent.com/CorvinLabs/Corvin-Marketplace/main/index/plugins.json` | Network fallback, used only when none of the above exists. |

The first three are checked for existence; the GitHub fetch runs from the
`FileNotFoundError` branch. If every source fails the API returns an empty index
(`count: 0`) rather than an error — the console then renders "No extensions
found", so an empty marketplace means *index not resolved*, not *no plugins*.
