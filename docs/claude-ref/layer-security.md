...
## Console hardening — adversarial round 2, 2026-09-07 (R2-C1..C5 + follow-ups)

All structural, all fail-closed, no flag and no env kill-switch.

| Finding | Where | Contract |
|---|---|---|
| **R2-C1** provider-supplied href | `routes/rag.py::_safe_source_url`, `shared/rag_query_engine.py::safe_http_url`, `web-next/src/lib/safe-url.ts` | `source_url` comes from a REMOTE RAG provider and was rendered as `<a href={item.source_url}>` — a hostile provider could return `javascript:` / `data:` / `vbscript:` and get script execution in the console origin. All three layers now allowlist an **absolute http(s) URL with a host**, reject control characters (a browser strips `\t\r\n` before parsing the scheme, so `java\nscript:` defeats a prefix check) and cap the length. Fail-closed: anything else renders as inert text. |
| **R2-C2** SSRF shared address space | `routes/datasources_http.py::_ip_is_blocked` (imported by `routes/custom_provider.py` — one predicate, no drift) | `is_private` is **False** for 100.64.0.0/10 (RFC 6598 CGNAT / Tailscale / cloud fabrics) on py3.11, so the guard let it through. The test is now `not ip.is_global` **plus** the explicit private/loopback/link-local/reserved/multicast/unspecified flags and the IMDS list. |
| **R2-C3** inbound webhook | `routes/webhooks.py` | Three gaps, all closed: (a) `hmac_secret_env` is **required at registration** (400 otherwise, env-var NAME shape validated) — a signature-less channel was an anonymous writer to the tenant's audit chain; a legacy manifest without one is refused at receive time (503), never accepted unauthenticated. (b) `rate_limit_per_hour` was persisted and never read — it is now **enforced** with a sliding window per `(tenant, channel)`, checked **before** the body is read, and a throttled request gets 429 with no chain write. (c) the body is **capped** at `_MAX_WEBHOOK_BODY_BYTES` (256 KiB, same order as `routes/memory.py`) via Content-Length and again while streaming, so a chunked body cannot buffer past the limit either. |
| **R2-C4** SPA asset fallback | `app.py::_is_asset_path` + `_SPAStaticFiles.get_response` | A missing `assets/*.js` was answered with **200 text/html** (the SPA shell), so a browser holding a stale shell executed HTML as a script — a blank page that reads exactly like the stale-bundle failure mode `console-deploy.sh` exists to surface. Anything whose first path segment is `assets` now 404s; client-side ROUTES still fall back to `index.html` with `no-cache`, and existing hashed assets keep `immutable`. |
| **R2-C5** malformed tenant | `routes/webhooks.py::receive_webhook` | `POST /v1/console/webhook/__evil/<chan>` raised `InvalidTenantID` into a **500** (stack trace) for an unauthenticated caller. The tenant id is validated and the channel id shape-checked first; anything unknown or malformed is a plain **404**. |
| follow-up: RAG-hub import | `routes/rag_hub.py::_assert_manifest_endpoints_allowed` | `/import` writes a provider manifest into the SAME registry dir as `custom_provider.create`, and that manifest's endpoint is what the RAG layer later fetches server-side — but only `create` ran the SSRF/egress guard. The import path now runs the same guard over every `endpoint`/`url`/`base_url`/`host` value at any depth of the manifest, before it is written. |
| follow-up: DNS rebinding | `routes/custom_provider.py::_validated_provider_pin` + `_pinned_request_url` | The guard resolved the host and then handed the NAME to httpx, which resolved it **again** at connect time — a ~0-TTL record answers a public IP to the guard and `169.254.169.254` to the connect. `test-api` now connects to the ONE validated address while carrying the original hostname in the `Host` header and TLS SNI (`extensions={"sni_hostname": …}`), mirroring what `datasources_http`'s IP-pinned urllib opener already did. Loopback and literal-IP endpoints need no pin. |
| follow-up: geo precision | `core/geo/instance_locator.py::GeoCoordinate.__post_init__` / `.quantize` | A Tier-3 record advertises "10km quantized", but only `grid_cell_id` was quantised — `to_dict()` served the RAW telemetry lat/lon to the world-map API and the on-disk cache. Coordinates are snapped to the 0.1° grid at **construction**, so every consumer sees the coarse value and a cache file written before the fix is re-quantised on reload. |

Regression tests: `core/console/tests/test_webhook_hardening_r2.py`,
`core/console/tests/test_console_r2_hardening.py`,
