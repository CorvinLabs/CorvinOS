...
| Chain-LOSS acknowledgement (`python -m forge.chain_loss --chain <canonical> --cause "…" [--backup <surviving copy>]… --i-understand-the-chain-is-lost`) — when the live chain file is DESTROYED and no backup of its tail survives, `chain_replaced` refuses every boot by design and "restore the chain" is impossible. The only admissible way forward is this explicit operator act (no env var, no flag, no boot path; refuses unless the anchored identity really differs from the live file and no other process holds it open): the post-loss file is RENAMED (`audit.jsonl.postloss-<ts>`, under the chain lock), the lost chain's path-keyed identity + witness are MOVED to `<key dir>/{chain_ids,chain_witness}_retired/`, and a fresh chain starts with a CRITICAL `audit.chain_loss_acknowledged` record naming the lost genesis + last anchored tail + cause, followed by `audit.chain_supersedes` seams to the post-loss file and every supplied backup. Nothing is deleted, merged or rewritten. First used 2026-09-24 after `git clean -fxd` in the checkout holding CORVIN_HOME destroyed the chain; the host-wide Claude hook `~/.claude/hooks/block_runtime_wipe.sh` now refuses `git clean -x/-X`, `git stash --all` and recursive removal of `.corvin` in every Claude session | L16 / ADR-2058 | GDPR Art. 30, 32 | ✅ Locked |
| Console READERS traverse seam-linked history; VERIFIERS do not. `security_events.chain_history_files(chain)` resolves every forward `audit.chain_supersedes` seam of the canonical chain (and, transitively, the seams inside those files) to the regular file in `chain.parent/` or `chain.parent/recovered/` whose `chain_path_key` equals the seam's `superseded_key`, oldest first by first-record ts; `.corrupt-` fragments stay linked for auditors but never feed a dashboard; the resolution is cached on (canonical size+mtime, both directories' size+mtime). `iter_chain_records(chain)` yields history then canonical records. Aggregating console readers use it (`model_usage`, `/learning/audit`, learning-loop audit rows, `maturity_live`, the engine-config classified tally, `model_selection_learner` cost/thresholds); the usage epoch (ADR-0760) filters history records per event like any other. The tripwire, `verify_chain`, the daily verify and compliance reports read the canonical chain ALONE — history is read, never merged, rewritten, reordered or deleted (`tests/security/test_audit_chain_history_readers.py`) | L16 / ADR-2058 | GDPR Art. 30, 32 | ✅ Locked |
...
| **Telemetry transparency panel** — `/console/app/otel-telemetry` renders every outbound channel from THAT CHANNEL'S OWN state (`aco/telemetry/last_ping`, `aco/telemetry/heartbeat_state.json`, `healing-traces/` + `sent/` + `.last_upload`, `aco/telemetry/outbox` + `sent/`, the opt-out flags the senders read) via `GET /v1/console/telemetry/channels` (`routes/telemetry_overview.py`). It shows the payload the sender builds now, the field allowlists, the endpoint host, cadence, last/next send; tokens are reported present/absent only. A channel nothing calls says `not_wired`; a channel that only collects says `collected_never_sent` — on the maintainer host the error-signature outbox held 10 164 reports (since 2026-07-26) that nothing ever submitted. Every sender records its outcomes (timestamps, HTTP status or exception class, counters — never the payload) in a state file the panel reads | ADR-0900 | GDPR Art. 5(1)(a) transparency, Art. 6(1)(f) | ✅ Live |
| **OTLP metrics, feature-stability digest, error-signature batches** (default-ON, opt-out; CONTENT-FREE, validated on BOTH ends). OTLP/HTTP push of the four `corvin.instance.*` gauges after every heartbeat (`aco/otel_bridge.py`, opt-out `ping_enabled`); hourly ADR-0288 digest of flag evaluation/error counts (`aco/stability_sender.py`, fed by `feature_flags.is_enabled`, opt-out `ping_enabled`); the ADR-0179 outbox folded into ONE hourly batch (`telemetry.submit_batch`, opt-out `consent_granted`). Receivers in Corvin-Features (`/v1/telemetry/feature-stability`, `/error-signatures`, `/otlp/v1/metrics`) authenticate with the HMAC token pair, rate-limit per instance, drop unknown fields and refuse free text / PII. The gateway now starts the hourly ping + nightly upload loops the launcher used to start. **Do NOT** widen a field allowlist on one side without the other, carry a message or a path outside the repo, or add a periodic push the panel cannot see | ADR-0901 | GDPR Art. 6(1)(f) legitimate interest | ✅ Live |
...
| Regulator-facing PDF reports sanitise at ONE chokepoint (`corvin_compliance_reports/sanitize.py`): every chain- or caller-derived value is escaped before it enters a reportlab `Paragraph` (`pdf_markup`) or a `Table` cell / canvas string / PDF metadata field (`pdf_text`); `tenant_id` is a CLOSED vocabulary and is rejected, not escaped, when it fails `[a-z0-9_][a-z0-9_-]{0,62}`. A crafted `hash` in the chain used to repaint the report's hash-chain anchor, and unbalanced markup used to make all three reports ungeneratable (2026-09-07) | ADR-0017 / ADR-0656 | EU AI Act Art. 50 · GDPR Art. 30 | ✅ Locked |
