...
class LLMSynthesisStage:
    ...
    def run(self, bundle, ctx):
        ...
        try:
            out = subprocess.run(
                # R4: whole-payload guard - `bundle.task` is the user's raw
                # turn and the retrieved-context digest is recalled user text;
                # both are interleaved with the framing template above, and the
                # payload rides POSITIONALLY in argv. Reply contract
                # unaffected: the JSON envelope is parsed from stdout.
                [_resolve_bin(), "-p", _guard_prompt_head(prompt),
