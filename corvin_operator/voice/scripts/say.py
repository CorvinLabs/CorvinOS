...
def _use_os_trust_store() -> None:
    ...
    `SSL_CERT_FILE` / `REQUESTS_CA_BUNDLE` are NOT an alternative here: edge-tts
    builds its context as `ssl.create_default_context(cafile=certifi.where())`
    (`edge_tts/communicate.py`), and an explicit `cafile=` overrides those env
    vars. Injection is the only lever that reaches it without patching the dep.

    The console process does its own injection in
    `core/console/corvin_console/standalone.py::_use_os_trust_store`, which does
    NOT cover this file: say.py runs as a separate SUBPROCESS, so it must anchor
    its own trust. "Console fine, TTS silent" is the expected shape of that gap.
...
# a TLS-inspecting corporate proxy. Guard: tests/test_voice_tls_trust_store.py.
_use_os_trust_store()
