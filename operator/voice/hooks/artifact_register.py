...
def _generate_description(path: Path, mime: str) -> str:
    ...
    try:
        ...
        # R4: whole-payload guard - the describe prompt embeds an artifact
        # FILENAME chosen by whoever produced the artifact, which is exactly a
        # `@<path>`-shaped injection slot. Reply contract unaffected: the first
        # stdout line is taken as the description.
        r = subprocess.run(
            argv, input=_guard_prompt_head(prompt),
