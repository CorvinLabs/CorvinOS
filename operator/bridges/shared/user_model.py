...
def _default_judge(prompt: str, timeout_s: float, bin_path: str) -> str:
    ...
    try:
        # R4: whole-payload guard — the distillation prompt embeds recalled
        # user turns, and a STDIN prompt is expanded by the CLI exactly like a
        # positional one. Reply contract unaffected: `_extract_json_object`
        # parses stdout.
        proc = subprocess.run(
