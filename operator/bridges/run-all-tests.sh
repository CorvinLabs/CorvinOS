...
run "Python: task progress (intermediate updates→outbox)" bash -c 'PYTEST="${PYTEST:-}"; [[ -z "$PYTEST" ]] && { echo "(skip: pytest not found)"; exit 0; }; "$PYTEST" shared/test_task_progress.py -q >/dev/null 2>&1' || fails=$((fails+1))
run "Python: task supervisor (heal stopped runs)"         bash -c 'PYTEST="${PYTEST:-}"; [[ -z "$PYTEST" ]] && { echo "(skip: pytest not found)"; exit 0; }; "$PYTEST" shared/test_task_supervisor.py -q >/dev/null 2>&1' || fails=$((fails+1))
run "Python: supervised bg worker (real subprocess E2E)"  bash -c 'PYTEST="${PYTEST:-}"; [[ -z "$PYTEST" ]] && { echo "(skip: pytest not found)"; exit 0; }; "$PYTEST" shared/test_bg_task_worker_supervised.py -q >/dev/null 2>&1' || fails=$((fails+1))
...
run "Node: outbox poller (preCheck+dedup+dead-letter+stall)" node shared/js/test_outbox_poller.js >/dev/null || fails=$((fails+1))
run "Node: discord outbox hardening (stall visibility + snowflake guard)" node discord/test_outbox_hardening.js >/dev/null || fails=$((fails+1))
...
run "Node: discord /task slash gate (F-B2)" node discord/test_slash_task_gate.js >/dev/null || fails=$((fails+1))
run "Node: discord execution-context embed" node discord/test_execution_context_embed.js >/dev/null || fails=$((fails+1))
