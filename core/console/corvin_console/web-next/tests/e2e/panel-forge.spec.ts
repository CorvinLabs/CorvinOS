/**
 * Forge Panel E2E Tests
 *
 * Auto-generated test suite for forge panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Forge Panel', () => {
  const panelSlug = 'forge';
  const panelTitle = 'Forge';

  test('navigates to forge and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/forge/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/forge/**', (route) => {
      route.abort('failed');
    });
