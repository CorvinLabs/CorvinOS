/**
 * Dashboard Panel E2E Tests
 *
 * Auto-generated test suite for dashboard panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Dashboard Panel', () => {
  const panelSlug = 'dashboard';
  const panelTitle = 'Dashboard';

  test('navigates to dashboard and loads', async ({ panelNav }) => {
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/dashboard/**', (route) => {
      route.abort('failed');
    });
