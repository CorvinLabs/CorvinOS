/**
 * Orgs Panel E2E Tests
 *
 * Auto-generated test suite for orgs panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Orgs Panel', () => {
  const panelSlug = 'orgs';
  const panelTitle = 'Orgs';

  test('navigates to orgs and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/orgs/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/orgs/**', (route) => {
      route.abort('failed');
    });
