/**
 * Marketplace Hub Panel E2E Tests
 *
 * Auto-generated test suite for marketplace-hub panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Marketplace Hub Panel', () => {
  const panelSlug = 'marketplace-hub';
  const panelTitle = 'Marketplace Hub';

  test('navigates to marketplace-hub and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/marketplace-hub/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/marketplace-hub/**', (route) => {
      route.abort('failed');
    });
