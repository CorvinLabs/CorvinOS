/**
 * Compute Panel E2E Tests
 *
 * Auto-generated test suite for compute panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Compute Panel', () => {
  const panelSlug = 'compute';
  const panelTitle = 'Compute';

  test('navigates to compute and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/compute/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/compute/**', (route) => {
      route.abort('failed');
    });
