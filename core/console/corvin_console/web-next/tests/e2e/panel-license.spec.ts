/**
 * License Panel E2E Tests
 *
 * Auto-generated test suite for license panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('License Panel', () => {
  const panelSlug = 'license';
  const panelTitle = 'License';

  test('navigates to license and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/license/);
  });
