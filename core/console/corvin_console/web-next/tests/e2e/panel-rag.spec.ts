 ...
 * Rag Panel E2E Tests
 *
 * Auto-generated test suite for rag panel.
 */
...
test.describe('Rag Panel', () => {
  const panelSlug = 'rag';
  const panelTitle = 'Rag';

  test('navigates to rag and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/rag/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/rag/**', (route) => {
      route.abort('failed');
    });
