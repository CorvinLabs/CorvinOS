/**
 * Api Keys Panel E2E Tests
 *
 * Auto-generated test suite for api-keys panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Api Keys Panel', () => {
  const panelSlug = 'api-keys';
  const panelTitle = 'Api Keys';

  test('navigates to api-keys and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/api-keys/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/api-keys/**', (route) => {
      route.abort('failed');
    });
