/**
 * Compliance Panel E2E Tests
 *
 * Auto-generated test suite for compliance panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Compliance Panel', () => {
  const panelSlug = 'compliance';
  const panelTitle = 'Compliance';

  test('navigates to compliance and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/compliance/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/compliance/**', (route) => {
      route.abort('failed');
    });
