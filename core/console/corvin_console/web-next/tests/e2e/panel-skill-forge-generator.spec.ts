/**
 * Skill Forge Generator Panel E2E Tests
 *
 * Auto-generated test suite for skill-forge-generator panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Skill Forge Generator Panel', () => {
  const panelSlug = 'skill-forge-generator';
  const panelTitle = 'Skill Forge Generator';

  test('navigates to skill-forge-generator and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/skill-forge-generator/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/skill-forge-generator/**', (route) => {
      route.abort('failed');
    });
