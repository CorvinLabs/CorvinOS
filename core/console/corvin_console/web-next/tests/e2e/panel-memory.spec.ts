 ...
 *
 * Auto-generated test suite for memory panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Memory Panel', () => {
  const panelSlug = 'memory';
  const panelTitle = 'Memory';

  test('navigates to memory and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/memory/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/memory/**', (route) => {
      route.abort('failed');
    });
