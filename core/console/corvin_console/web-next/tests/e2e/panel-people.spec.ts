/**
 * People Panel E2E Tests
 *
 * Auto-generated test suite for people panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('People Panel', () => {
  const panelSlug = 'people';
  const panelTitle = 'People';

  test('navigates to people and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/people/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/people/**', (route) => {
      route.abort('failed');
    });
