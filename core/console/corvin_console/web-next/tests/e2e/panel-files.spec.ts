/**
 * Files Panel E2E Tests
 *
 * Auto-generated test suite for files panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Files Panel', () => {
  const panelSlug = 'files';
  const panelTitle = 'Files';

  test('navigates to files and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/files/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/files/**', (route) => {
      route.abort('failed');
    });
