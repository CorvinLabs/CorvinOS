/**
 * Packages Panel E2E Tests
 *
 * Auto-generated test suite for packages panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Packages Panel', () => {
  const panelSlug = 'packages';
  const panelTitle = 'Packages';

  test('navigates to packages and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/packages/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/packages/**', (route) => {
      route.abort('failed');
    });
