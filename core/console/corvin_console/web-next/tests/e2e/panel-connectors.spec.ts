/**
 * Connectors Panel E2E Tests
 *
 * Auto-generated test suite for connectors panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Connectors Panel', () => {
  const panelSlug = 'connectors';
  const panelTitle = 'Connectors';

  test('navigates to connectors and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/connectors/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/connectors/**', (route) => {
      route.abort('failed');
    });
