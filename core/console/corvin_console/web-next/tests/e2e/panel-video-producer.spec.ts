/**
 * Video Producer Panel E2E Tests
 *
 * Auto-generated test suite for video-producer panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Video Producer Panel', () => {
  const panelSlug = 'video-producer';
  const panelTitle = 'Video Producer';

  test('navigates to video-producer and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/video-producer/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/video-producer/**', (route) => {
      route.abort('failed');
    });
