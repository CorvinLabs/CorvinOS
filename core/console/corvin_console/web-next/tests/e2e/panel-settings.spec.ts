 ...
 *
 * Auto-generated test suite for settings panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Settings Panel', () => {
  const panelSlug = 'settings';
  const panelTitle = 'Settings';

  test('navigates to settings and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/settings/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/settings/**', (route) => {
      route.abort('failed');
    });
