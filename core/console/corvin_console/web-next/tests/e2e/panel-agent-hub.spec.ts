/**
 * Agent Hub Panel E2E Tests
 *
 * Auto-generated test suite for agent-hub panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Agent Hub Panel', () => {
  const panelSlug = 'agent-hub';
  const panelTitle = 'Agent Hub';
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/agent-hub/**', (route) => {
      route.abort('failed');
    });
