/**
 * Skills Panel E2E Tests
 *
 * Auto-generated test suite for skills panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Skills Panel', () => {
  const panelSlug = 'skills';
  const panelTitle = 'Skills';

  test('navigates to skills and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/skills/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/skills/**', (route) => {
      route.abort('failed');
    });
