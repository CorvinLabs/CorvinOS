/**
 * Custom Provider Panel E2E Tests
 *
 * Auto-generated test suite for custom-provider panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Custom Provider Panel', () => {
  const panelSlug = 'custom-provider';
  const panelTitle = 'Custom Provider';

  test('navigates to custom-provider and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/custom-provider/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/custom-provider/**', (route) => {
      route.abort('failed');
    });
