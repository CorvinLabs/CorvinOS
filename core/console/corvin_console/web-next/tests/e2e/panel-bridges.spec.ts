/**
 * Bridges Panel E2E Tests
 *
 * Auto-generated test suite for bridges panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Bridges Panel', () => {
  const panelSlug = 'bridges';
  const panelTitle = 'Bridges';

  test('navigates to bridges and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/bridges/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/bridges/**', (route) => {
      route.abort('failed');
    });
