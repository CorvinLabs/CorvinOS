 ...
 * Ldd Panel E2E Tests
 *
 * Auto-generated test suite for ldd panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Ldd Panel', () => {
  const panelSlug = 'ldd';
  const panelTitle = 'Ldd';

  test('navigates to ldd and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/ldd/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/ldd/**', (route) => {
      route.abort('failed');
    });
