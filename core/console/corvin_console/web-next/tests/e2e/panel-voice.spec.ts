/**
 * Voice Panel E2E Tests
 *
 * Auto-generated test suite for voice panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Voice Panel', () => {
  const panelSlug = 'voice';
  const panelTitle = 'Voice';

  test('navigates to voice and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/voice/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/voice/**', (route) => {
      route.abort('failed');
    });
