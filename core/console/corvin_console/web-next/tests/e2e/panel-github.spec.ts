/**
 * Github Panel E2E Tests
 *
 * Auto-generated test suite for github panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Github Panel', () => {
  const panelSlug = 'github';
  const panelTitle = 'Github';

  test('navigates to github and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/github/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/github/**', (route) => {
      route.abort('failed');
    });
