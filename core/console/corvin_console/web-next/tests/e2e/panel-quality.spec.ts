/**
 * Quality Panel E2E Tests
 *
 * Auto-generated test suite for quality panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Quality Panel', () => {
  const panelSlug = 'quality';
  const panelTitle = 'Quality';

  test('navigates to quality and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/quality/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/quality/**', (route) => {
      route.abort('failed');
    });
