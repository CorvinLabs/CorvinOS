/**
 * Otel Telemetry Panel E2E Tests
 *
 * Auto-generated test suite for otel-telemetry panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Otel Telemetry Panel', () => {
  const panelSlug = 'otel-telemetry';
  const panelTitle = 'Otel Telemetry';

  test('navigates to otel-telemetry and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/otel-telemetry/);
  });
  ...
  test('handles API errors gracefully', async ({ page, panelNav }) => {
    await page.route('**/v1/console/otel-telemetry/**', (route) => {
      route.abort('failed');
    });
