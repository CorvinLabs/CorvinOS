/**
 * Workflows Panel E2E Tests
 *
 * Auto-generated test suite for workflows panel.
 */

import { test, expect } from '../fixtures/panel-fixtures';

test.describe('Workflows Panel', () => {
  const panelSlug = 'workflows';
  const panelTitle = 'Workflows';

  test('navigates to workflows and loads', async ({ panelNav }) => {
    await panelNav.goto(panelSlug);
    await panelNav.assertLoaded(panelSlug);
    expect(panelNav.page).toHaveURL(/\/app\/workflows/);
  });
