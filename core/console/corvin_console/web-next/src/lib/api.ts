 ...
 * "no audio" (provider down). Either way the caller (playFull()) stops the
 * playlist. 2026-08-04: `null` for `index === 0` still means "an absent
 * enhancement" (no TTS/LLM backend at all) — but for any later index it
 * means a segment the server already promised (via an earlier segment's own
 * `total`) failed to synthesize; playFull() surfaces THAT case via onError,
 * since the true end-of-playlist never needs a 204 (the caller's own
 * `index < total` loop bound already stops the next fetch beforehand).
 */
