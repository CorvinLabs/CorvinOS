  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-6">
        <h3 className="text-sm font-semibold mb-4 text-[#C9D1D9] flex items-center gap-2">
          ...
          {MOCK_ROUTING_PATTERNS.map((pattern, i) => (
            <div key={i}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  <span className="text-sm font-medium text-[#C9D1D9]">{pattern.engine}</span>
                  <span className="text-xs text-[#8B949E] ml-2">({pattern.count} calls)</span>
                </div>
                <span className={`text-xs font-semibold ${pattern.trend.startsWith('+') ? 'text-[#3FB950]' : 'text-[#FB8500]'}`}>
                  {pattern.trend}
                </span>
              </div>
              <div className="w-full bg-[#0D1117] rounded-full h-2 overflow-hidden">
                <div
                  className="bg-gradient-to-r from-[#58A6FF] to-[#79C0FF] h-2 rounded-full"
                  style={{ width: `${pattern.percentage}%` }}
                ></div>
              </div>
              <div className="text-xs text-[#8B949E] mt-1">{pattern.percentage}% of all routing decisions</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-6">
        <h3 className="text-sm font-semibold mb-4 text-[#C9D1D9] flex items-center gap-2">
          <BarChart3 size={18} className="text-[#79C0FF]" />
          Top Skills Used (7d)
        </h3>
          ...
          {MOCK_SKILL_PATTERNS.map((skill, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-[#0D1117] rounded border border-[#30363D]">
              <div className="flex-1">
                <div className="text-sm font-medium text-[#C9D1D9]">{skill.skill}</div>
                <div className="text-xs text-[#8B949E] mt-0.5">
                  {skill.count} invocations • {skill.category}
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm font-semibold text-[#58A6FF]">{skill.percentage}%</div>
                <div className="w-16 h-1.5 bg-[#161B22] rounded mt-1 overflow-hidden">
                  <div
                    className="bg-[#58A6FF] h-1.5 rounded"
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-6">
        <h3 className="text-sm font-semibold mb-4 text-[#C9D1D9] flex items-center gap-2">
          <FolderOpen size={18} className="text-[#79C0FF]" />
        ...
        <div className="space-y-4">
          {MOCK_CONTEXT_BUCKETS.map((bucket, i) => (
            <div key={i}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-[#C9D1D9]">{bucket.bucket}</span>
                <span className="text-xs font-semibold text-[#8B949E]">{bucket.count} ({bucket.percentage}%)</span>
              </div>
              <div className="w-full bg-[#0D1117] rounded-full h-2 overflow-hidden">
                <div
      ...
      {/* Pattern Insights */}
      <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-6">
        <h3 className="text-sm font-semibold mb-4 text-[#C9D1D9]">📌 Pattern Insights</h3>
        <ul className="space-y-3 text-sm text-[#C9D1D9]">
          ...
          <li className="flex gap-3">
            <span className="text-[#79C0FF] flex-shrink-0">→</span>
