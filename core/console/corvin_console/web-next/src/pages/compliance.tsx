          ...
          {(isCritical || isWarning) && (
            <Badge
              ...
              className={cn(
                ...
                isCritical ? "border-destructive text-destructive" : "border-amber-500 text-amber-600 dark:text-amber-400",
              )}
            >
              {ev.severity}
            </Badge>
                ...
                <div key={k} className="flex items-baseline gap-1.5 min-w-0">
                  ...
                  <span
                    className={cn(
                      ...
                      typeof v === "boolean"
                        ? v ? "text-emerald-600 dark:text-emerald-400" : "text-destructive"
                        : "text-foreground/80",
                    )}
                  >
                    {v === null || v === undefined
      ...
      <CardContent className="space-y-3 pt-0">
        ...
        {q.data && events.length > 0 && (
          <>
            ...
            {/* chain event list */}
            <div className="rounded-md border border-border bg-card py-2 overflow-hidden">
              {events.map((ev, i) => (
                <ChainEventRow
                  key={`${ev.ts}-${i}`}
                  ev={ev}
