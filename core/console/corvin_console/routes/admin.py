...
def _entries(tenant_id: str) -> dict[str, _Entry]:
    ...
    Declared plugins (``spec.plugins.installed`` in tenant.corvin.yaml) are NOT
    folded in here: the block that tried to did so with keyword arguments
    ``PluginRecord`` does not have, raised TypeError on every entry, and swallowed
    it — dead code that only looked like a fallback (finding F-P6). A declared
    plugin appears through the runtime view below once it has loaded.
    """
