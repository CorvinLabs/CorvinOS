...
# NOTE: onboarding is now marked complete by the installer itself
# (corvinOS/installer/core.py step_18_finalise writes _SETUP_COMPLETE_PATH +
# onboarding.json directly) rather than by a "Finish" click in a first-run
# wizard — the console UI no longer gates the chat behind onboarding, so
# there is no in-app moment left to POST a completion signal from.
