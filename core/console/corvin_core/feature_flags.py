...
REGISTRY: tuple[FeatureFlag, ...] = (
    ...
    FeatureFlag(
        ...
        description=(
            ...
            "(default, ship-dark) = every turn keeps using the resolved OS model, "
            "exactly as before. On = `resolve_model_for_workload` may downgrade a "
            "high-confidence chat turn; an explicit model pin always wins, and any "
            "non-chat or low-confidence workload is untouched (fail-closed). Every "
            "routing decision is audited with workload/confidence/model only — never "
            "message content. Deliberately NO env-var switch: a process-wide flag "
            "would cross tenant boundaries and bypass the operator's opt-in."
        ),
        owner="maintainer",
        target_release="0.12.x",
        tags=("models", "delegation", "cost"),
    ),
    FeatureFlag(
