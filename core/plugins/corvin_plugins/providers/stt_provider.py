"""STTProvider registry (ADR-0030 plugin type ``stt_provider``, L23).

``stt_provider`` was listed in ``KNOWN_PLUGIN_TYPES`` from the start but had no
...
Compliance note (L23): transcription audit records METADATA ONLY — never the
transcript text (CLAUDE.md § Compliance Baseline, GDPR Art. 5).  A provider that
puts recognised text into an audit detail or a log line breaks that guarantee.
"""
...
_log = logging.getLogger("corvin.stt.provider")


class STTProviderRegistry:
    """Holds the active STT provider for this process.  Thread-safe.

    No default implementation: absence means "the built-in STT chain applies"
    (L23 resolves its own chain), not "transcription is off".
    """
...
_registry: STTProviderRegistry = STTProviderRegistry()
