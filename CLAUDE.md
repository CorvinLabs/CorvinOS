...
| Tier 2/3 geo-tracking (region/city, opt-IN only — requires config tier AND explicit consent; Cloudflare-edge-resolved, never a raw IP; file-based TTL 30d/14d) | GDPR Art. 6(1)(a) explicit consent | ADR-0205/0206 (`aco/htrace_consent.py::geo_tracking_tier`/`geo_tracking_consent_given`, `corvin_features/telemetry/geo_tiers.py`) |
