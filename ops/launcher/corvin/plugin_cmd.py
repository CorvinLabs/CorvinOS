...
def _manifest_yaml(plugin_id: str, plugin_type: str, version: str) -> str:
    ...
    return (
        ...
        f"# The key is `boot_layer`, never `layer` — the scaffold emitted the\n"
        f"# legacy key until 2026-07-28, so `corvin plugin check` warned about\n"
        f"# its own freshly generated manifest on the very first run.\n"
        f"boot_layer: installed\n"
        f"origin: community\n"
        f"\n"
