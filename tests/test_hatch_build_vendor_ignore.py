...
to force-included files — the vendored `corvin_operator/*` subtrees are copied via
...
real dev/CI artifacts (confirmed present today: corvin_operator/bridges/.pytest_cache,
corvin_operator/bridges/shared/.pytest_cache, corvin_operator/bridges/.ldd/heartbeat) could
...
(`corvin_operator/voice/scripts/Testnachricht mit Nova.`) that happened to be sitting
...
def test_voice_i18n_bundle_dir_is_vendored():
    ...
    assert "corvin_operator/voice/i18n" in sources, (
        f"corvin_operator/voice/i18n missing from _VENDOR_MAP -- wheel installs "
    ...
    # relative to the vendored corvin_operator/bridges/shared/i18n.py, so the
    # destination must land at .../_vendor/corvin_operator/voice/i18n exactly.
    dest = dict(hb._VENDOR_MAP)["corvin_operator/voice/i18n"]
    assert dest == "corvin_console/_vendor/corvin_operator/voice/i18n", dest
...
def test_bundle_skills_dir_is_vendored():
    ...
    corvin_operator/bundle/config-templates was ever in _VENDOR_MAP -- the 12 LDD
    ...
    (or `corvin pkg install ./corvin_operator/bundle/`) ever seeded
    ~/.claude/skills/, exactly the same failure class as the i18n bug
    above."""
    sources = [src for src, _dest in hb._VENDOR_MAP]
    for required in (
        "corvin_operator/bundle/skills",
        "corvin_operator/bundle/install.sh",
        "corvin_operator/bundle/manifest.yaml",
    ):
    ...
    assert dest_map["corvin_operator/bundle/skills"] == "corvin_console/_vendor/corvin_operator/bundle/skills"
    assert dest_map["corvin_operator/bundle/install.sh"] == "corvin_console/_vendor/corvin_operator/bundle/install.sh"
    assert dest_map["corvin_operator/bundle/manifest.yaml"] == "corvin_console/_vendor/corvin_operator/bundle/manifest.yaml"
...
def test_ignore_callback_excludes_untracked_file_and_untracked_dir(tmp_path):
    ...
    `corvin_operator/bridges/coworktest/` shape) sitting next to real vendored
