...
class TestWorkerRegistry:
    ...
    def test_register_worker(self):
        ...
        assert registry.get("w1") == metrics
    ...
    def test_tenant_isolation(self):
        ...
        assert len(tenant1_workers) == 3
        assert all(w.tenant_id == "tenant1" for w in tenant1_workers)
...
class TestMetricsCollector:
    ...
    def test_clear_metrics(self):
        ...
        # Clear w1
        collector.clear("w1")
        assert collector.get_avg_latency("w1") is None
        assert collector.get_avg_latency("w2") is not None
...
class TestAlertSystem:
    ...
    def test_alert_on_critical_cpu(self):
        ...
        assert alerts[0].metric_name == "cpu_percent"
...
class TestWorkerMonitor:
    ...
    def test_register_and_monitor_worker(self):
        ...
        assert monitor.registry.count() == 1
    ...
    def test_worker_status_computation(self):
        ...
        assert status3["status"] == "unhealthy"
    ...
    def test_cluster_status_aggregation(self):
        ...
        assert cluster["healthy"] + cluster["degraded"] + cluster["unhealthy"] == 3
    ...
    def test_export_metrics_json(self):
        ...
        assert "cluster_status" in export
        assert "workers" in export
        assert len(export["workers"]) == 1
        assert export["workers"][0]["worker_id"] == "w1"
    ...
    def test_deregister_worker(self):
        ...
        assert monitor.registry.count() == 0
    ...
    def test_latency_percentiles(self):
        ...
        assert status["latency_p99_ms"] is not None
        # p99 should be higher than p50
        assert status["latency_p99_ms"] >= status["latency_p50_ms"]
...
class TestWorkerSimulation:
    ...
    def test_simulated_healthy_worker(self):
        ...
        assert status["status"] == "healthy"
        assert status["latency_p50_ms"] is not None
        assert 50 <= status["latency_p50_ms"] <= 60
...
class TestGlobalMonitor:
    ...
    def test_tenant_isolation_in_global_factory(self):
        ...
        assert m1.registry.count("tenant1") == 1
        assert m2.registry.count("tenant2") == 1


class TestE2EWorkerMonitor:
    ...
    def test_worker_lifecycle_e2e(self):
        ...
        assert status["status"] == "healthy"
        assert status["metrics"]["tasks_completed"] == 50
    ...
    def test_cluster_monitoring_e2e(self):
        ...
        cluster = monitor.get_cluster_status()
        assert cluster["worker_count"] == 10
        assert cluster["healthy"] == 7
        assert cluster["degraded"] >= 2
